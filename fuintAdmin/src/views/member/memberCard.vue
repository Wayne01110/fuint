<template>
  ...
  <el-dialog title="微信会员卡设置" :visible.sync="showDialog" class="common-dialog" width="800px" @close="cancel" destroy-on-close>
    <el-form ref="cardForm" :model="cardForm" :rules="rules" label-width="180px">
      ...
      <el-row>
        <el-col :span="9">
          <el-form-item label="会员卡背景：" prop="backgroundUrl">
            <el-upload
              :action="uploadAction"
              list-type="picture-card"
              :class="{ hide:hideUpload }"
              ...
              <img
                v-if="this.cardForm.backgroundUrl"
                :src="imagePath + this.cardForm.backgroundUrl"
                class="list-img"
              />
              <i v-if="!this.cardForm.backgroundUrl" class="el-icon-plus"></i>
            </el-upload>
          </el-form-item>
        </el-col>
        <p class="form-tips">（提示：点击图片修改，建议尺寸：1000 x 600）</p>
      </el-row>
      <el-row>
        <el-col :span="24">
